...
public class CalendarUtil {
    static List<DateStateBean> dateStateBeanList=null;
    /**
     * 获得当月显示的日期（上月 + 当月 + 下月）
     *
     * @param year  当前年份
    ...
    public static List<DateBean> getMonthDate(int year, int month, Map<String, String> map) {
        ...
        SharedPrefrenceUtils.setInt(UIUtils.getContext(),"week",week);
        int lastYear;
        int lastMonth;
        if (month == 1) {
            lastMonth = 12;
...
//        }
    ...
    public static  void setDateState(List<DateStateBean> dateStateBeans){
        dateStateBeanList=dateStateBeans;
    }
    public static void clearDateState(){
        if(dateStateBeanList!=null){
            dateStateBeanList.clear();
        }
    }

    public static List<DateStateBean> getDateStateBeanList() {
        return dateStateBeanList;
    }
