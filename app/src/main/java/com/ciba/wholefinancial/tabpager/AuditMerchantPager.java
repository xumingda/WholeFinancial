 ...
 * @更新内容: TODO
 */
public class AuditMerchantPager extends ViewTabBasePager implements
        PullToRefreshBase.OnRefreshListener {
    // 分类listview
    @ViewInject(R.id.lv_audit_merchant)
    private PullToRefreshListView lv_audit_merchant;
    ...
    public AuditMerchantPager(Context context, boolean isFirst) {
        super(context);
    ...
    protected View initView() {
        View view = View.inflate(mContext,
                R.layout.audit_merchant_pager, null);
    ...
    public void initData() {
        ...
        lv_audit_merchant.setMode(PullToRefreshBase.Mode.BOTH);
        lv_audit_merchant.getLoadingLayoutProxy(false, true).setPullLabel("上拉加载");
        lv_audit_merchant.getLoadingLayoutProxy(false, true).setRefreshingLabel("加载中");
        lv_audit_merchant.getLoadingLayoutProxy(false, true).setReleaseLabel("放开以刷新");
        lv_audit_merchant.setOnRefreshListener(this);
...
//        }
    ...
    private void setData() {
        ...
        if (merchantAdapter == null) {
            merchantAdapter = new MerchantAdapter( mContext, merchantBeanList,1);
            lv_audit_merchant.setAdapter(merchantAdapter);
...
//                            SharedPrefrenceUtils.setString(mContext, url + "3", json);
                ...
                if (isRefresh) {
                    LogUtils.e("结束");
                    isRefresh = false;
                    lv_audit_merchant.onRefreshComplete();
                }
            }
    ...
    public void onRefresh(PullToRefreshBase refreshView) {
        if (!isRefresh) {
            isRefresh = true;
            if (lv_audit_merchant.isHeaderShown()) {
                pageNo = 1;
                merchantBeanList.clear();
                getMerchantList();
            } else if (lv_audit_merchant.isFooterShown()) {
                pageNo++;
                getMerchantList();
            }
        }
