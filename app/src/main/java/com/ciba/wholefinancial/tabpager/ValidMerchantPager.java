 ...
 * @更新内容: TODO
 */
public class ValidMerchantPager extends ViewTabBasePager implements
        PullToRefreshBase.OnRefreshListener {
    // 分类listview
    @ViewInject(R.id.lv_vaild_merchant)
    private PullToRefreshListView lv_vaild_merchant;
    ...
    public ValidMerchantPager(Context context, boolean isFirst) {
        super(context);
    ...
    protected View initView() {
        View view = View.inflate(mContext,
                R.layout.vaild_merchant_pager, null);
    ...
    public void initData() {
        ...
        lv_vaild_merchant.setMode(PullToRefreshBase.Mode.BOTH);
        lv_vaild_merchant.getLoadingLayoutProxy(false, true).setPullLabel("上拉加载");
        lv_vaild_merchant.getLoadingLayoutProxy(false, true).setRefreshingLabel("加载中");
        lv_vaild_merchant.getLoadingLayoutProxy(false, true).setReleaseLabel("放开以刷新");
        lv_vaild_merchant.setOnRefreshListener(this);
...
//        }
    ...
    private void setData() {
        ...
        if (merchantAdapter == null) {
            ...
            lv_vaild_merchant.setAdapter(merchantAdapter);
...
//                            SharedPrefrenceUtils.setString(mContext, url + "3", json);
                ...
                if (isRefresh) {
                    LogUtils.e("结束");
                    isRefresh = false;
                    lv_vaild_merchant.onRefreshComplete();
                }
            }
    ...
    public void onRefresh(PullToRefreshBase refreshView) {
        if (!isRefresh) {
            isRefresh = true;
            if (lv_vaild_merchant.isHeaderShown()) {
                pageNo = 1;
                merchantBeanList.clear();
                getMerchantList();
            } else if (lv_vaild_merchant.isFooterShown()) {
                pageNo++;
                getMerchantList();
            }
        }
