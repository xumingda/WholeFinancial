...
import com.ciba.wholefinancial.request.GetMerchantRequest;
 ...
 * @更新内容: TODO
 */
public class AllMerchantPager extends ViewTabBasePager implements
        PullToRefreshBase.OnRefreshListener {
    // 分类listview
    @ViewInject(R.id.lv_all_merchant)
    private PullToRefreshListView lv_all_merchant;
    ...
    public AllMerchantPager( Context context, boolean isFirst) {
        super(context);
    ...
    protected View initView() {
        View view = View.inflate(mContext,
                R.layout.all_merchant_pager, null);
    ...
    public void initData() {
        ...
        lv_all_merchant.setMode(PullToRefreshBase.Mode.BOTH);
        lv_all_merchant.getLoadingLayoutProxy(false, true).setPullLabel("上拉加载");
        lv_all_merchant.getLoadingLayoutProxy(false, true).setRefreshingLabel("加载中");
        lv_all_merchant.getLoadingLayoutProxy(false, true).setReleaseLabel("放开以刷新");
        lv_all_merchant.setOnRefreshListener(this);
...
//        }
        ...
        lv_all_merchant.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
                Intent intent=new Intent(mContext,MerchantInfoActivity.class);
                intent.putExtra("businessId",merchantBeanList.get(i-1).getId());
    ...
    private void setData() {
        ...
        if (merchantAdapter == null) {
            ...
            lv_all_merchant.setAdapter(merchantAdapter);
            LogUtils.e("设置数据"+merchantBeanList.size());
        } else {
            merchantAdapter.setDate(merchantBeanList);
...
//        params.put("status", "1");
        ...
        MyVolley.uploadNoFile(MyVolley.POST, url, params, new MyVolley.VolleyCallback() {
            ...
            public void dealWithJson(String address, String json) {
                ...
                LogUtils.e("getMerchantListResponsessss:" + json);
...
//                            SharedPrefrenceUtils.setString(mContext, url + "3", json);
                ...
                if (isRefresh) {
                    LogUtils.e("结束");
                    isRefresh = false;
                    lv_all_merchant.onRefreshComplete();
                }
            }
    ...
    public void onRefresh(PullToRefreshBase refreshView) {
        if (!isRefresh) {
            isRefresh = true;
            if (lv_all_merchant.isHeaderShown()) {
                pageNo = 1;
                merchantBeanList.clear();
                getMerchantList();
            } else if (lv_all_merchant.isFooterShown()) {
                pageNo++;
                getMerchantList();
            }
        }
